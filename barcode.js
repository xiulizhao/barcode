(function(root, factory) {
  ...
  var loadDependence = function(fncallback) {
    ...
    if (!window.wwload.barcode) {
      window.wwload.barcode = "wait";
      // requirejs.config({
      //   paths: {
      //     "插件名": "插件路径" // 示例: libs/qrcodejs/qrcode
      //   }
      // });
      require(["libs/JsBarcode/dist/JsBarcode.all.min"], function(barcode) {
        window.wwload.barcode = true;
        window.barcode=barcode;
        replace();
        fncallback();
      });
    } else if (window.wwload.barcode === "wait") {
      setTimeout(function () {
   ...
   */
   ...
   function barcode1($ele){
    var config = {};
    config.width = eval($ele.attr("data-width"));  //设置条形码宽度,宽度选项是单个条的宽度。例如4
    config.height = eval($ele.attr("data-height")); //设置条形码的高度.例如50
    config.fontSize = eval($ele.attr("data-fontsize")); //设置条形码的字体大小,例如24
    config.lineColor = $ele.attr("data-linecolor"); //设置条形码的颜色,例如#0fc
    config.displayValue = eval($ele.attr("data-displayvalue") || "true"); 
    config.text = $ele.attr("data-text");             //条形码替换的文字
    config.fontOptions = $ele.attr("data-fontoptions");  //条形码字体添加粗体斜体等,例如blod,italic,bold italic.
    config.textAlign = $ele.attr("data-textalign");      //设置文本的水平对齐。可以左/中/右 例如:left/center/right.
    config.textPosition  = $ele.attr("data-textposition");  //设置文本的垂直位置。可以是底部/顶部 例如:bottom/top
    config.textMargin = $ele.attr("data-textmargin");    //设置条形码和文本之间的空间。
    config.background = $ele.attr("data-background");    //设置条形码的背景。
    config.barcodeNumber = $ele.attr("data--barcodenumber");     //条形码编号

   $ele.find(".barcode").JsBarcode(config.barcodeNumber,{
      width: config.width,
  ...
  function processElement($ele) {
    ...
    setTimeout(function(){
      $.wwclass.helper.updateProp($ele, "data-x-inited", true);
     //图片隐藏取消
     $ele.find("img").removeClass("hidden");
    },100);
